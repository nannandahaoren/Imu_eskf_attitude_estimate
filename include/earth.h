...
const double WGS84_E1 = 0.0066943799901413156; /* 第一偏心率平方 小e*/
const double WGS84_E2 = 0.0067394967422764341; /* 第二偏心率平方 */

const double lat = 47.32;  // 纬度
const double lon = 123.77; // 经度
const double alt = 148.0;    // 高程altitude

const double lat_rad = 0.8258898020437168;  // 纬度
const double lon_rad = 2.1601940151933814; // 经度
...
public:
    ...
    /* 正常重力计算，纬度和经度传入角度，不是弧度 */
    static double gravity(const double lat_rad, const double lon_rad, const double alt)
    {
        // 传进来纬度，经度，高度
        // 转化成弧度
     ...
     * @brief
     * @param blh
     * @return g_w_v_n_I  由 w_ie_n  v_n 组成矩阵的转置的逆，不移动（走动）为常值矩阵Matrix3d
     * @return g_w_v_n_I Matrix3d
     */
    static Matrix3d cal_g_w_v_n_I(const Vector3d &blh)
    {
        double g = gravity(blh);
        Eigen::Matrix3d g_w_v_n_I;
        g_w_v_n_I << tan(blh[0]) / g, 1 / (WGS84_WIE * cos(blh[0])), 0,
            0, 0, 1 / (g * WGS84_WIE * cos(blh[0])),
            1 / g, 0, 0;
        // std::cout << g_w_v_n_I << std::endl;
        return g_w_v_n_I;
    }
